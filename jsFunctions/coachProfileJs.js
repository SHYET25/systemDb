$(document).ready(function() {
        ...
        data.forEach(function(athlete, index) {
            ...
            <div class="col-md-12 mb-4">
                <div class="card" style="max-width: 100%; height: 100%;">
                    <img src="images/prof_pics/${athlete.ath_img}" class="card-img img-responsive mx-auto d-block" alt="Athlete Image" style="height: 300px; width: 300px;">
                    <div class="card-body">
                        <h5 class="card-title text-center">${athlete.ath_name}</h5>
                        <p class="card-text"><strong>Position:</strong> ${athlete.ath_position}</p>
                        <p class="card-text"><strong>ID:</strong> ${athlete.AthleteID}</p>
                        <p class="card-text"><strong>Height:</strong> <span class="editable" data-ath-id="${athlete.AthleteID}" data-ath-field="height" data-ath-value="${athlete.ath_height}">${athlete.ath_height}</span></p>
                        <p class="card-text"><strong>Weight:</strong> <span class="editable" data-ath-id="${athlete.AthleteID}" data-ath-field="weight" data-ath-value="${athlete.ath_weight}">${athlete.ath_weight}</span></p>
                        <p class="card-text"><strong>Age:</strong> <span class="editable" data-ath-id="${athlete.AthleteID}" data-ath-field="age" data-ath-value="${athlete.ath_age}">${athlete.ath_age}</span></p>
    ...
    function populateAthleteTableHome(data) {
        ...
        data.forEach(function(athlete, index) {
            var card = `<div class="card mb-3 mx-auto" style="max-width: 90%;">
            ...
            $card.find('.editable').dblclick(function() {
                var $this = $(this);
                var fieldName = $this.data('ath-field');
                var fieldValue = $this.data('ath-value');
                var input = $(`<input type="text" class="edit-input" value="${fieldValue}" style="width: 50px;" />`);
                $this.html(input);
                input.focus();
    
                // Attach focusout event to update the field value
                input.off('focusout').on('focusout', function() {
                    var newValue = $(this).val().trim();
                    var athId = $this.data('ath-id');
    
                    // Perform validation if needed before updating (e.g., numeric values)
                    if (fieldName === 'height' || fieldName === 'weight' || fieldName === 'age') {
                        // Example validation for numeric values
                        if (isNaN(newValue)) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "phpFile/buttonFunctions/updateField.php",
                        data: { ath_id: athId, field: fieldName, value: newValue },
                        dataType: 'json',
                        success: function(response) {
                            if (response.status === 'success') {
                                $this.text(newValue);
                                $this.data('ath-value', newValue);
                                reloadAllFunctions(); // You may define this function to reload necessary functionalities
                            } else {
...
;
    ...
    function populateTableMatch(athleteTableBody, data) {
        ...
        data.forEach(function(athlete) {
            var card = `
                <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
                    <div class="card" style="max-width: 100%; height: 100%;">
                        <img src="images/prof_pics/${athlete.ath_img}" class="card-img" alt="Athlete Image" style="height: 250px; object-fit: cover;">
        ...
        $('.editable').off('dblclick').on('dblclick', function() {
            ...
            var input = $(`<input type="text" class="edit-input" value="${teamName}" class="h2 fw-bold" style="width 100%" />`);
