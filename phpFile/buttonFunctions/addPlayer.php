...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $game_quarter = intval($_POST['game_quarter']);
    ...
    // Check if player is already in the game for the specified number
    $checkQuery = "SELECT COUNT(*) as count, ath_name FROM basketball_game_tracking JOIN athlete_info ON athlete_info.AthleteID = basketball_game_tracking.ath_bball_player_id WHERE ath_bball_player_id = ?";
    $checkStmt = $conn->prepare($checkQuery);
    $checkStmt->bind_param("i", $ath_bball_player_id);
    $checkStmt->execute();
    $result = $checkStmt->get_result();
    $row = $result->fetch_assoc();
    $count = $row['count'];
    $athleteName = $row['ath_name'];
