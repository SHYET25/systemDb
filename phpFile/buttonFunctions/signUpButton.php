...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // If the email or username already exists in the selected sport's table, return an error message
    if ($result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            ...
            // Insert into basketball_athlete_info table
            $table = '';
            switch ($sports) {
                case 'basketball':
                    ...
                    if ($general_stmt->execute()) {
                        ...
                        if ($percentage_stmt->execute()) {
                            // Insert into basketball_team_main table with team status
                            $team_status = '--'; // Determine team status based on ranking if needed
                            $team_sql = "INSERT INTO basketball_team_main (ath_id, ath_team) VALUES (?, ?)";
                            $team_stmt = $conn->prepare($team_sql);
                            $team_stmt->bind_param("is", $last_id, $team_status);

                            if ($team_stmt->execute()) {
                                echo json_encode(['status' => 'success']);
                            } else {
                                echo json_encode(['status' => 'error', 'message' => 'Unable to record data in basketball_team_main table']);
                                error_log('Insert operation failed in basketball_team_main: ' . $team_stmt->error);
                            }
